...
class NoticiasPipeline:
   ...
   def store_db(self, item):    
        self.curr.execute("""insert into noticia_tb values (?,?,?,?,?,?)""",(str(item['id'][0]) ,str(item['title'][0]),str(item['subtitulo'][0]),str(item['url'][0]),str(item['data'][0]),str(item['image_url'][0])))              
        self.conn.commit()
